...
function questionInput(question: Question, value: unknown, onChange: (value: unknown) => void, disabled: boolean) {
  const baseClass = "min-h-10 rounded border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 disabled:bg-slate-100";
  ...
  if (["multiple_choice", "symptom_checklist", "exposure_history"].includes(question.question_type)) {
    ...
    return (
      <div className="grid gap-2">
        {options.map((option) => (
          <label className="inline-flex items-center gap-2 text-sm text-slate-700" key={option}>
            <input
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-200 p-4">
        <div>
          <h2 className="text-base font-bold text-slate-950">{response.template_name || response.question_snapshot.name}</h2>
          <p className="mt-1 text-xs font-semibold text-slate-500">v{response.template_version} · {response.respondent_role.replaceAll("_", " ")}</p>
        </div>
      ...
      <div className="grid gap-5 p-4">
          ...
          <div className="grid gap-3" key={section.key}>
            ...
            <div className="grid gap-3">
              {section.questions.map((question) => (
                <label className="grid gap-1 rounded border border-slate-200 bg-slate-50 p-3" key={question.key}>
                  <span className="text-sm font-bold text-slate-800">{question.label}{question.required ? <span className="text-rose-600"> *</span> : null}</span>
                  {question.help_text ? <span className="text-xs text-slate-500">{question.help_text}</span> : null}
        ...
        <div className="flex flex-wrap gap-2">
          {mode === "complete" ? (
            <>
              <button className="inline-flex h-10 items-center gap-2 rounded border border-slate-200 px-3 text-sm font-bold text-slate-700 disabled:opacity-50" disabled={busy || locked} onClick={() => onSave?.(values)} type="button"><Save size={16} /> Save draft</button>
              <button className="inline-flex h-10 items-center gap-2 rounded bg-brand-green px-3 text-sm font-bold text-white disabled:bg-slate-300" disabled={busy || locked} onClick={() => onSubmit?.(values)} type="button"><Send size={16} /> Submit</button>
            </>
          ) : (
            <button className="inline-flex h-10 items-center gap-2 rounded bg-brand-deep px-3 text-sm font-bold text-white disabled:bg-slate-300" disabled={busy || !["submitted", "resubmitted", "under_review"].includes(response.status)} onClick={onValidate} type="button"><Check size={16} /> Validate</button>
          )}
        </div>
      </div>
    </section>
