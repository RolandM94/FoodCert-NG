...
export function PhysicalExamForm({
  ...
  return (
    <div className="grid gap-3">
      {riskFlag ? (
        <div className="flex items-start gap-2 rounded border border-warning-100 bg-warning-50 p-3 text-sm font-semibold text-amber-900">
      ...
      {EXAM_FIELDS.map(([field, text]) => (
        <label className="flex items-center justify-between gap-3 rounded border border-neutral-200 bg-neutral-50 px-3 py-2 text-sm font-semibold text-neutral-700" key={field}>
          {text}
      ...
      <textarea className="min-h-24 rounded border border-neutral-200 bg-neutral-50 p-3 text-sm" disabled={disabled || busy} placeholder="Other notes" value={value.other_notes} onChange={(event) => updateField("other_notes", event.target.value)} />
      <div className="flex flex-wrap gap-2">
        <button className="inline-flex h-10 w-fit items-center gap-2 rounded border border-neutral-200 px-4 text-sm font-bold text-neutral-700 disabled:opacity-60" disabled={disabled || busy || completed} type="button" onClick={onSaveDraft}>
          <Save size={16} /> Save draft
        </button>
        <button className="inline-flex h-10 w-fit items-center gap-2 rounded bg-brand-600 px-4 text-sm font-bold text-white disabled:opacity-60" disabled={disabled || busy || completed} type="button" onClick={onComplete}>
