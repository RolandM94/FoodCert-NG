...
export function NotificationDashboard() {
  ...
  if (isError || !stats) {
    return <p className="rounded bg-rose-50 p-4 text-sm font-semibold text-rose-700">Could not load dashboard.</p>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-6">
        ...
        <StatCard label="Email" value={stats.emails_today} icon={Mail} color="bg-blue-50 text-blue-600" />
        <StatCard label="SMS" value={stats.sms_today} icon={Smartphone} color="bg-purple-50 text-purple-600" />
        <StatCard label="WhatsApp" value={stats.whatsapp_today} icon={MessageSquare} color="bg-emerald-100 text-emerald-700" />
        <StatCard label="Success Rate" value={stats.delivery_success_rate} icon={CheckCircle2} color="bg-emerald-50 text-brand-deep" />
        <StatCard label="Failed" value={stats.failed_deliveries} icon={RefreshCw} color="bg-rose-50 text-rose-600" />
        <StatCard label="Pending Retries" value={stats.pending_retries} icon={RefreshCw} color="bg-amber-50 text-amber-600" />
        <StatCard label="Critical" value={stats.critical_sent} icon={Radio} color="bg-red-50 text-red-600" />
      ...
      {topCategories.length > 0 ? (
        <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-4 flex items-center gap-2">
            <BarChart3 className="text-brand-deep" size={18} />
            <h3 className="text-base font-bold text-slate-950">Top Categories</h3>
          </div>
          <div className="space-y-2">
            {topCategories.map(([cat, count]) => (
              <div key={cat} className="flex items-center gap-3">
                <span className="w-32 text-sm font-semibold text-slate-700 capitalize">{cat.replaceAll("_", " ")}</span>
                <div className="flex-1 h-5 rounded bg-slate-100 overflow-hidden">
                  <div className="h-full rounded bg-brand-green" style={{ width: `${Math.min(100, (count / Math.max(1, topCategories[0][1])) * 100)}%` }} />
                </div>
