  ...
  return (
    <PortalShell role="lab_staff" title="Test Request Detail" description="Collect sample, enter result values, upload result documents, and submit to the doctor.">
      <div className="grid gap-5">
        <Link className="inline-flex w-fit items-center gap-2 rounded border border-neutral-200 bg-white px-3 py-2 text-sm font-bold text-neutral-700 shadow-sm" href="/lab/test-requests"><ArrowLeft size={16} /> Back to requests</Link>
        ...
        {request ? (
          <>
            <section className="grid gap-3 md:grid-cols-4">
              ...
              <div className="rounded-lg border border-neutral-200 bg-white p-4 shadow-sm"><p className="text-xs font-bold uppercase text-neutral-500">Test</p><p className="mt-2 font-bold capitalize text-neutral-900">{request.test_name || label(request.test_type)}</p></div>
            ...
            <section className="grid gap-4 lg:grid-cols-2">
              <div className="rounded-lg border border-neutral-200 bg-white p-5 shadow-sm">
                ...
                <div className="mt-4 grid gap-3">
                  <textarea className="min-h-20 rounded border border-neutral-200 bg-neutral-50 p-3 text-sm" placeholder="Lab staff notes" value={form.lab_staff_notes} onChange={(event) => setForm((current) => ({ ...current, lab_staff_notes: event.target.value }))} />
                  <button className="inline-flex h-10 w-fit items-center gap-2 rounded border border-neutral-200 px-4 text-sm font-bold text-neutral-700 disabled:opacity-60" disabled={busy} type="button" onClick={() => void collectSample()}><Save size={16} /> Mark sample collected</button>
                  <select className="h-10 rounded border border-neutral-200 bg-white px-3 text-sm" value={form.status} onChange={(event) => setForm((current) => ({ ...current, status: event.target.value }))}>{RESULT_STATUSES.map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select>
                  <input className="h-10 rounded border border-neutral-200 bg-neutral-50 px-3 text-sm" placeholder="Result value" value={form.result_value} onChange={(event) => setForm((current) => ({ ...current, result_value: event.target.value }))} />
                  <textarea className="min-h-20 rounded border border-neutral-200 bg-neutral-50 p-3 text-sm" placeholder="Result notes" value={form.result_notes} onChange={(event) => setForm((current) => ({ ...current, result_notes: event.target.value }))} />
                  <button className="inline-flex h-10 w-fit items-center gap-2 rounded bg-brand-600 px-4 text-sm font-bold text-white disabled:opacity-60" disabled={busy} type="button" onClick={() => void saveResult()}><Save size={16} /> Save result</button>
                  <button className="inline-flex h-10 w-fit items-center gap-2 rounded border border-brand-300 px-4 text-sm font-bold text-brand-800 disabled:opacity-60" disabled={busy || !["positive", "negative", "inconclusive", "repeat_required", "result_uploaded"].includes(request.status)} type="button" onClick={() => void submitToDoctor()}><Save size={16} /> Submit to doctor</button>
                </div>
              </div>
              <div className="rounded-lg border border-neutral-200 bg-white p-5 shadow-sm">
                <h2 className="text-sm font-bold text-neutral-900">Result Document</h2>
                <div className="mt-4 grid gap-3">
                  <input className="rounded border border-neutral-200 bg-neutral-50 p-3 text-sm" type="file" accept="application/pdf,image/png,image/jpeg" onChange={(event) => setFile(event.target.files?.[0] || null)} />
                  <button className="inline-flex h-10 w-fit items-center gap-2 rounded bg-brand-600 px-4 text-sm font-bold text-white disabled:opacity-60" disabled={busy || !file} type="button" onClick={() => void uploadDocument()}><Upload size={16} /> Upload document</button>
