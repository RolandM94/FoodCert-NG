    ...
    <PortalShell role="lab_staff" title="Test Requests" description="Collect samples, enter results, upload result documents, and submit requests for doctor review.">
      <div className="grid gap-5">
        ...
        <section className="flex items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-sm font-bold text-slate-950">{rows.length} requests</p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-200">
                {rows.length ? rows.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td className="p-3 capitalize">
                      <p className="font-semibold text-slate-900">{row.test_name || label(row.test_type)}</p>
                      {row.parent_lab_test ? <p className="text-xs font-semibold text-amber-700">Repeat</p> : null}
                      {row.is_flagged ? <p className="text-xs font-semibold text-amber-700">Flagged</p> : null}
                    </td>
                ...
                )) : <tr><td className="p-3 text-slate-500" colSpan={5}>No requests yet.</td></tr>}
              </tbody>
            </table>
          </div>
        </section>
