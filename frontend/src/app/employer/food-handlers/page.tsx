  ...
  return (
    <PortalShell role="employer" title="Food Handlers" description="View and manage all food handlers linked to your business.">
      <div className="mb-5 grid gap-3 lg:grid-cols-[minmax(220px,1fr)_repeat(5,auto)]">
        <label className="flex h-10 min-w-[200px] items-center gap-2 rounded border border-slate-200 bg-white px-3">
        ...
        <label className="flex items-center gap-2 text-sm font-semibold text-slate-700">
          ...
          <select className="h-10 rounded border border-slate-200 bg-white px-2 text-xs" value={branchFilter} onChange={(e) => setBranchFilter(e.target.value)}>
            <option value="">All branches</option>
            {branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}
          </select>
        </label>
        <select className="h-10 rounded border border-slate-200 bg-white px-2 text-xs font-semibold text-slate-700" value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)}>
          {CATEGORY_OPTIONS.map((c) => <option key={c.value} value={c.value}>{c.label}</option>)}
        </select>
        <select aria-label="Fitness status" className="h-10 rounded border border-slate-200 bg-white px-2 text-xs font-semibold text-slate-700" value={fitnessFilter} onChange={(e) => setFitnessFilter(e.target.value)}>
        ...
        <select aria-label="Certificate status" className="h-10 rounded border border-slate-200 bg-white px-2 text-xs font-semibold text-slate-700" value={certificateFilter} onChange={(e) => setCertificateFilter(e.target.value)}>
        ...
        <select aria-label="Expiry window" className="h-10 rounded border border-slate-200 bg-white px-2 text-xs font-semibold text-slate-700" value={expiryWindow} onChange={(e) => setExpiryWindow(e.target.value)}>
        ...
        <div className="mb-5 grid gap-3 sm:grid-cols-4">
          {[
            ...
            <div key={label as string} className="rounded-lg border border-slate-200 bg-white p-3 text-center">
              <p className="text-xl font-bold text-slate-950">{count as number}</p>
      ...
      {loading && <div className="rounded-lg border border-slate-200 bg-white p-8 text-center text-sm text-slate-500">Loading food handlers...</div>}
      {error && !loading && <div className="rounded-lg border border-rose-200 bg-rose-50 p-4 text-sm font-semibold text-rose-700">{error}</div>}

      {!loading && !error && handlers.length === 0 && (
        <div className="rounded-lg border border-slate-200 bg-white p-10 text-center">
          <UsersRound size={32} className="mx-auto text-slate-300" />
          <p className="mt-3 text-sm font-semibold text-slate-500">No food handlers found</p>
          <p className="mt-1 text-xs text-slate-400">Adjust filters, invite food handlers, or link existing certified handlers.</p>
        </div>
      )}
          ...
          <table className="w-full min-w-[1160px] text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50 text-left">
                <th className="px-4 py-3 text-xs font-bold uppercase text-slate-500">Handler</th>
                <th className="px-4 py-3 text-xs font-bold uppercase text-slate-500">Branch</th>
                <th className="px-4 py-3 text-xs font-bold uppercase text-slate-500">Category</th>
                <th className="px-4 py-3 text-xs font-bold uppercase text-slate-500">Fitness</th>
                <th className="px-4 py-3 text-xs font-bold uppercase text-slate-500">Certificate</th>
                <th className="px-4 py-3 text-xs font-bold uppercase text-slate-500">Typhoid</th>
                <th className="px-4 py-3 text-xs font-bold uppercase text-slate-500">Hep. A</th>
                <th className="px-4 py-3 text-xs font-bold uppercase text-slate-500">Last Assessment</th>
              ...
              {handlers.map((h) => (
                <tr key={h.id} className="hover:bg-slate-50/50">
                  <td className="px-4 py-3">
                    <p className="font-semibold text-slate-900">{h.full_name}</p>
                    <p className="text-xs text-slate-500">{h.phone}</p>
                  </td>
                  <td className="px-4 py-3"><span className="text-xs text-slate-600">{h.business_branch_name || "—"}</span></td>
                  <td className="px-4 py-3"><span className="text-xs text-slate-600 capitalize">{h.food_handler_category?.replace(/_/g, " ") || "—"}</span></td>
                  <td className="px-4 py-3"><FitnessStatusBadge status={h.fitness_status} /></td>
                  <td className="px-4 py-3">
                    {h.certificate_number ? (
                      <div>
                        <p className="text-xs font-mono text-slate-700">{h.certificate_number}</p>
                        {h.certificate_expiry_date && <p className="text-[10px] text-slate-400">Exp: {new Date(h.certificate_expiry_date).toLocaleDateString()}</p>}
                      </div>
                  ...
                  <td className="px-4 py-3"><span className="text-xs text-slate-500">{h.last_assessment_date ? new Date(h.last_assessment_date).toLocaleDateString() : "—"}</span></td>
                  <td className="px-4 py-3">
                    <div className="flex min-w-[220px] flex-wrap items-center gap-2">
                      <button className="inline-flex h-8 items-center gap-1.5 rounded border border-slate-200 px-2 text-xs font-bold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50" disabled={!h.certificate_number} onClick={() => h.certificate_number && router.push(`/verify/${encodeURIComponent(h.certificate_number)}`)} type="button">
                        <FileBadge size={13} /> Verify
                      </button>
                      <button className="inline-flex h-8 items-center gap-1.5 rounded border border-slate-200 px-2 text-xs font-bold text-slate-700 hover:bg-slate-50" onClick={() => router.push(`/employer/illness-reports?handler=${h.id}`)} type="button">
                        <HeartPulse size={13} /> Report
                      </button>
                      <select aria-label={`Assign branch for ${h.full_name}`} className="h-8 max-w-[150px] rounded border border-slate-200 bg-white px-2 text-xs font-semibold text-slate-600" disabled={updatingHandler === h.id} onChange={(e) => handleBranchChange(h.id, e.target.value)} value={h.business_branch || ""}>
