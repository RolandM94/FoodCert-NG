      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center justify-between gap-3 border-b border-slate-100 p-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-wide text-brand-deep">Eligible Assessments</p>
              <h2 className="mt-1 text-base font-bold text-slate-950">Select records to pay</h2>
            </div>
                  ...
                  <tr key={assessment.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3">
                      <input
                        ...
                        className="h-4 w-4 rounded border-slate-300 text-brand-deep"
                        onChange={() => toggleAssessment(assessment.id)}
                        type="checkbox"
                      />
                    </td>
                    <td className="px-4 py-3">
                      <p className="font-semibold text-slate-950">{assessment.food_handler_name || "Unnamed handler"}</p>
                      <p className="text-xs text-slate-500">{assessment.food_handler_identifier || assessment.id}</p>
                    </td>
                    <td className="px-4 py-3 text-slate-600">{assessment.facility_name || "Not assigned"}</td>
                    <td className="px-4 py-3">
                      <span className="rounded bg-amber-50 px-2 py-1 text-xs font-bold uppercase text-amber-700 ring-1 ring-amber-200">
                        {assessment.status.replaceAll("_", " ")}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-slate-500">{new Date(assessment.created_at).toLocaleDateString()}</td>
                  </tr>
                ))}
        ...
        <aside className="grid content-start gap-4">
          <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-center gap-2">
              <Calculator className="text-brand-deep" size={18} />
              <h2 className="text-base font-bold text-slate-950">Quote</h2>
            </div>
            <div className="mt-4 grid gap-3 text-sm">
              <div className="flex items-center justify-between gap-3">
                <span className="text-slate-500">Selected assessments</span>
                <span className="font-bold text-slate-950">{selectedAssessments.length}</span>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span className="text-slate-500">Quoted total</span>
                <span className="font-bold text-slate-950">{quote ? money(quote.amount) : "Not quoted"}</span>
              </div>
            </div>
            <button
              className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded bg-brand-deep px-3 py-2 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50"
            ...
            {quoteMutation.isError ? (
              <p className="mt-3 text-sm font-semibold text-rose-600">Quote could not be generated. Refresh the list and try again.</p>
            ) : null}
          </div>
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="mt-4 max-h-72 space-y-3 overflow-y-auto">
                {quote.line_items.map((item) => (
                  <div key={item.assessment_id} className="rounded border border-slate-100 p-3">
                    <p className="text-sm font-bold text-slate-950">{item.food_handler_name}</p>
                    <p className="mt-1 text-xs text-slate-500">{item.facility_name}</p>
                    <p className="mt-2 text-sm font-bold text-brand-deep">{money(item.amount)}</p>
                  </div>
                ))}
              </div>
              <button
                className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded bg-brand-green px-3 py-2 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50"
                disabled={busy}
          ...
          {payment ? (
            <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-5 text-brand-deep">
              <div className="flex items-center gap-2">
