...
function ScoreBadge({ score }: { score?: string | null }) {
  if (!score) return <span className="text-sm font-semibold text-slate-400">Not scored</span>;
  const value = Number(score);
  const tone = value >= 80 ? "text-brand-deep" : value >= 60 ? "text-amber-700" : "text-rose-700";
...
function InspectionRow({ inspection }: { inspection: EmployerInspectionSummary }) {
  return (
    <tr>
      <td className="border-b border-slate-50 py-4 pr-4 align-top">
        <p className="font-semibold text-slate-950">{formatDate(inspection.inspection_date)}</p>
        <p className="mt-1 text-xs text-slate-500">{inspection.inspector_name || "Inspector"}</p>
      </td>
      <td className="border-b border-slate-50 py-4 pr-4 align-top">
        <p className="font-semibold text-slate-800">{inspection.branch_name || "All branches"}</p>
        <p className="mt-1 line-clamp-2 max-w-xl text-sm leading-6 text-slate-600">{inspection.findings_summary || "No findings recorded."}</p>
      </td>
      <td className="border-b border-slate-50 py-4 pr-4 align-top">
      ...
      <td className="border-b border-slate-50 py-4 pr-4 align-top">
        ...
        <p className="mt-2 text-xs font-semibold capitalize text-slate-500">{inspection.enforcement_action.replaceAll("_", " ")}</p>
      </td>
      <td className="border-b border-slate-50 py-4 pr-4 align-top text-sm text-slate-600">{inspection.response_count}</td>
      <td className="border-b border-slate-50 py-4 text-right align-top">
        <Link
          className="inline-flex h-9 items-center gap-2 rounded border border-slate-200 px-3 text-sm font-bold text-brand-deep hover:bg-emerald-50"
      ...
      <div className="grid gap-6">
        <section className="grid gap-4 md:grid-cols-3">
          <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <p className="text-xs font-bold uppercase tracking-wide text-brand-deep">Reports</p>
            <p className="mt-2 text-3xl font-bold text-slate-950">{inspections.length}</p>
          </div>
          <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <p className="text-xs font-bold uppercase tracking-wide text-brand-deep">Need Response</p>
            <p className="mt-2 text-3xl font-bold text-amber-700">{needsResponse}</p>
          </div>
          <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <p className="text-xs font-bold uppercase tracking-wide text-brand-deep">Average Score</p>
            <p className="mt-2 text-3xl font-bold text-brand-deep">{averageScore}%</p>
          </div>
        </section>

        <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-4 flex items-center gap-2">
            <Filter className="text-brand-deep" size={18} />
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-bold text-slate-950">Inspection History</h2>
            </div>
            <span className="text-sm font-semibold text-slate-500">{inspectionsQuery.isFetching ? "Loading..." : `${inspections.length} shown`}</span>
          </div>
          {inspectionsQuery.isError ? (
            <div className="rounded-lg bg-rose-50 p-4 text-sm font-semibold text-rose-700">Could not load inspection history.</div>
          ) : null}
          <div className="overflow-x-auto">
            <table className="w-full min-w-[820px] text-left text-sm">
              <thead className="text-xs font-bold uppercase tracking-wide text-slate-500">
                <tr>
                  <th className="border-b border-slate-100 py-2 pr-4">Date</th>
                  <th className="border-b border-slate-100 py-2 pr-4">Branch & Findings</th>
                  <th className="border-b border-slate-100 py-2 pr-4">Score</th>
                  <th className="border-b border-slate-100 py-2 pr-4">Status</th>
                  <th className="border-b border-slate-100 py-2 pr-4">Responses</th>
