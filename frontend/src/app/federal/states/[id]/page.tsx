        ...
        <div className="grid gap-5">
          <section className="grid gap-3 md:grid-cols-4">
            {[
              ...
              <div key={label as string} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <p className="text-xl font-bold text-slate-950">{value as string}</p>
              </div>
            ))}
          </section>
          ...
          <section className="grid gap-4 lg:grid-cols-2">
            <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
              <h2 className="mb-4 text-base font-bold text-slate-950">Operational Summary</h2>
              <div className="grid gap-3 text-sm">
                <div className="flex justify-between gap-3"><span className="text-slate-600">Approved facilities</span><strong>{numberLabel(state.approved_facilities)}</strong></div>
                <div className="flex justify-between gap-3"><span className="text-slate-600">Pending facility applications</span><strong>{numberLabel(state.pending_facility_applications)}</strong></div>
                <div className="flex justify-between gap-3"><span className="text-slate-600">Inspections</span><strong>{numberLabel(state.inspection_count)}</strong></div>
                <div className="flex justify-between gap-3"><span className="text-slate-600">Illness reports</span><strong>{numberLabel(state.illness_reports)}</strong></div>
                <div className="flex justify-between gap-3"><span className="text-slate-600">Latest report</span><StatusCell status={state.latest_report_status} /></div>
              </div>
            </div>

            <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
              <h2 className="mb-4 text-base font-bold text-slate-950">Privacy Boundary</h2>
          ...
          <DataTable
            columns={[
              { key: "type", header: "Report", render: (row) => <span className="font-bold text-slate-950">{row.report_type.replaceAll("_", " ")}</span> },
