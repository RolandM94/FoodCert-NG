...
const cardsConfig: Array<{ key: string; label: string; icon: typeof Flag; color: string }> = [
  ...
  { key: "inspections_this_month", label: "This Month", icon: TrendingUp, color: "text-brand-green" },
  { key: "open_enforcement_cases", label: "Open Cases", icon: Flag, color: "text-red-500" },
  { key: "total_notices_issued", label: "Notices Issued", icon: AlertTriangle, color: "text-orange-500" },
  { key: "critical_findings_national", label: "Critical Findings", icon: AlertTriangle, color: "text-red-600" },
      ...
      <div className="grid gap-5">
        <section className="grid gap-3 grid-cols-2 md:grid-cols-4">
          {cardsConfig.map(({ key, label, icon: Icon, color }) => (
            <div key={key} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <p className="mt-2 text-2xl font-bold text-slate-950">{String(Number(cards[key]) || 0)}</p>
            </div>
          ))}
        </section>
        ...
        {inspByState && inspByState.length > 0 && (
          <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            <h2 className="text-base font-bold text-slate-950 mb-3">Inspections by State</h2>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y">
                  {inspByState.map((s) => (
                    <tr key={s.employer__state__name} className="hover:bg-slate-50">
        ...
        {noticesByState && noticesByState.length > 0 && (
          <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            <h2 className="text-base font-bold text-slate-950 mb-3">Notices by State</h2>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 text-left text-xs uppercase text-slate-500">
