...
import { listEstablishmentCategories, listPolicyVersions } from "@/lib/api/standards";
import { CategoryFormDrawer } from "@/features/standards/category-form-drawer";
import type { EstablishmentCategory } from "@/types/standards";

export default function EstablishmentCategoriesPage() {
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [drawerMode, setDrawerMode] = useState<"create" | "edit">("create");
  const [editing, setEditing] = useState<EstablishmentCategory | null>(null);
  const [policyVersionId, setPolicyVersionId] = useState("");

  const { data, isLoading } = useQuery({
    queryKey: ["standards-establishment-categories"],
    queryFn: () => listEstablishmentCategories(),
  });
  ...
  function openEdit(row: EstablishmentCategory) {
  ...
  const columns: DataTableColumn<EstablishmentCategory>[] = [
    {
      key: "name",
    ...
    { key: "inspection_required", header: "Inspection Required", render: (row) => row.inspection_required ? "Yes" : "No" },
    {
      key: "status",
      header: "Status",
      render: (row) => (
  ...
  return (
    <StandardsPolicyWorkspaceShell workspace="assessment-standards"
      title="Establishment Categories"
      description="Define food establishment categories affected by the guideline."
    >
      <div className="grid gap-5">
        ...
        {isLoading ? (
          ...
          <DataTable<EstablishmentCategory>
            columns={columns}
            rows={rows}
            empty="No establishment categories configured for this policy version."
          />
        )}
      </div>
