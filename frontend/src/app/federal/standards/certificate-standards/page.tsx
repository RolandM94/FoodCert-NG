"use client";

import { useState } from "react";
import { useQuery } from "@tanstack/react-query";
import { StandardsPolicyWorkspaceShell } from "@/features/standards/standards-policy-workspaces";
import { DataTable, type DataTableColumn } from "@/components/ui/data-table";
import { listCertificateTemplates, listPolicyVersions } from "@/lib/api/standards";
import { CertificateTemplateFormDrawer } from "@/features/standards/certificate-template-form-drawer";
import type { CertificateTemplate } from "@/types/standards";

export default function CertificateStandardsPage() {
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [drawerMode, setDrawerMode] = useState<"create" | "edit">("create");
  const [editing, setEditing] = useState<CertificateTemplate | null>(null);
  const [policyVersionId, setPolicyVersionId] = useState("");

  const { data, isLoading } = useQuery({
    queryKey: ["standards-certificate-templates"],
    queryFn: () => listCertificateTemplates(),
  });

  const { data: versions } = useQuery({
    queryKey: ["standards-policy-versions"],
    queryFn: () => listPolicyVersions(),
  });

  const draftVersions = (Array.isArray(versions) ? versions : []).filter(
    (v) => v.status === "draft" || v.status === "returned"
  );
  const rows = Array.isArray(data) ? data : [];

  function openCreate() {
    if (draftVersions.length > 0) setPolicyVersionId(draftVersions[0].id);
    setEditing(null);
    setDrawerMode("create");
    setDrawerOpen(true);
  }

  function openEdit(row: CertificateTemplate) {
    setPolicyVersionId(row.policy_version);
    setEditing(row);
    setDrawerMode("edit");
    setDrawerOpen(true);
  }

  const columns: DataTableColumn<CertificateTemplate>[] = [
    {
      key: "template_name",
      header: "Template Name",
      render: (row) => (
        <button onClick={() => openEdit(row)} className="font-medium text-brand-700 hover:underline">
          {row.template_name}
        </button>
      ),
    },
    { key: "template_version", header: "Version", render: (row) => row.template_version },
    { key: "certificate_number_format", header: "Number Format", render: (row) => <code className="text-xs">{row.certificate_number_format}</code> },
    {
      key: "status",
      header: "Status",
      render: (row) => (
        <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${
          row.status === "active" ? "bg-brand-50 text-brand-700" :
          row.status === "draft" ? "bg-neutral-100 text-neutral-700" :
          "bg-neutral-100 text-neutral-500"
        }`}>
          {row.status.charAt(0).toUpperCase() + row.status.slice(1)}
        </span>
      ),
    },
    { key: "policy_version_code", header: "Policy Version", render: (row) => row.policy_version_code },
  ];

  return (
    <StandardsPolicyWorkspaceShell workspace="certification-facilities" title="Certificate Standards" description="Configure certificate templates, QR payload, and verification fields.">
      <div className="grid gap-5">        <section className="flex items-center justify-between rounded-lg border border-neutral-200 bg-white p-4 shadow-sm">
          <p className="text-sm text-neutral-600">{rows.length} templates</p>
          <button onClick={openCreate} disabled={draftVersions.length === 0} className="inline-flex h-10 items-center gap-2 rounded-md bg-brand-600 px-4 text-sm font-medium text-white hover:bg-brand-700 disabled:opacity-50">
            Create Template
          </button>
        </section>
        {draftVersions.length === 0 && (
          <div className="rounded border border-warning-100 bg-warning-50 p-3 text-sm text-warning-700">Create a draft policy version before adding certificate templates.</div>
        )}
        {isLoading ? <p className="text-sm text-neutral-500">Loading...</p> : (
          <DataTable<CertificateTemplate> columns={columns} rows={rows} empty="No certificate templates configured." />
        )}
      </div>
      <CertificateTemplateFormDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} onSuccess={() => setDrawerOpen(false)} mode={drawerMode} policyVersionId={policyVersionId} initial={editing} />
    </StandardsPolicyWorkspaceShell>
  );
}
