...
function statusClass(status: string) {
  ...
  if (status === "deprecated") return "bg-amber-50 text-amber-700 ring-amber-200";
    ...
    >
      <div className="space-y-5">
        ...
        <div className="flex flex-col justify-between gap-3 border-b border-slate-200 pb-4 sm:flex-row sm:items-end">
          <div>
            <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Organization type</p>
            <h2 className="mt-1 text-lg font-bold text-slate-950">
              {organization ? ORG_TYPE_LABELS[organization.organization_type] : "Loading"}
            </h2>
            <p className="mt-1 text-sm text-slate-600">{organization?.name ?? "Fetching organization profile."}</p>
          </div>
          <button
            className="inline-flex w-fit items-center gap-2 rounded bg-brand-green px-3 py-2 text-sm font-bold text-white hover:bg-brand-deep disabled:opacity-60"
        ...
        {showCreate ? (
          <form className="grid gap-3 rounded border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-3" onSubmit={handleCreate}>
            ...
            <div className="md:col-span-3">
              <button className="rounded bg-slate-950 px-3 py-2 text-sm font-bold text-white disabled:opacity-60" disabled={saving} type="submit">
        ...
        <div className="overflow-hidden rounded border border-slate-200 bg-white shadow-sm">
          <table className="w-full min-w-[760px] text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                <tr><td className="px-4 py-6 text-slate-500" colSpan={5}>Loading roles...</td></tr>
              ) : roles.map((role) => (
                <tr key={role.id}>
                  <td className="px-4 py-3">
                    <div className="font-bold text-slate-950">{role.name}</div>
                  ...
                  <td className="px-4 py-3 text-slate-700">{role.permission_count}</td>
                  <td className="px-4 py-3">
        ...
        {expandedRole ? (
          <section className="rounded border border-slate-200 bg-white p-4 shadow-sm">
            <div className="mb-4 flex items-center gap-2">
              <ShieldCheck aria-hidden="true" className="text-brand-deep" size={18} />
              <div>
                <h3 className="text-base font-bold text-slate-950">{expandedRole.name}</h3>
                <p className="text-sm text-slate-600">{expandedRole.description || "No description provided."}</p>
              </div>
            </div>
            <div className="grid gap-4 md:grid-cols-2">
              {Object.entries(permissionsByModule).map(([module, modulePermissions]) => (
                <div className="rounded border border-slate-100 p-3" key={module}>
                  ...
                  <div className="mt-3 space-y-2">
                    {modulePermissions.map((permission) => (
                      <div className="flex items-start justify-between gap-3 text-sm" key={permission.id}>
                        ...
                        <span className={`shrink-0 rounded px-2 py-1 text-xs font-bold ${rolePermissionCodes.has(permission.code) ? "bg-emerald-50 text-emerald-700" : "bg-slate-50 text-slate-400"}`}>
