...
function decisionBadge(decision: string): ReactNode {
  ...
  const map: Record<string, { color: string; label: string }> = {
    fit: { color: "bg-emerald-100 text-emerald-800", label: "Fit" },
    temporarily_not_fit: { color: "bg-amber-100 text-amber-800", label: "Temp Not Fit" },
    not_fit: { color: "bg-rose-100 text-rose-800", label: "Not Fit" },
    pending: { color: "bg-slate-100 text-slate-600", label: "Pending" },
  };
  const style = map[decision] || { color: "bg-slate-100 text-slate-600", label: decision };
      ...
      <div className="grid gap-5">
        ...
        <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center justify-between gap-3 border-b border-slate-200 p-4">
            <h2 className="text-sm font-bold text-slate-950">Pending Queue</h2>
            ...
            {data?.sections?.pending_queue?.length ? (
              data.sections.pending_queue.map((item, i) => (
                <div className="flex items-center justify-between gap-3 p-4 text-sm" key={i}>
                  <div><p className="font-bold text-slate-950">{item.food_handler || item.task}</p><p className="text-xs text-slate-500">{item.type || item.action}</p></div>
                  {item.status ? <StatusBadge status={item.status} /> : null}
                  {item.assessment_id ? <Link className="text-xs font-bold text-brand-green" href={`/doctor/assessments/${item.assessment_id}`}>Open</Link> : null}
                </div>
              ))
            ) : (
              <p className="p-4 text-sm text-slate-500">No pending items in your queue.</p>
            )}
          </div>
        </section>
            ...
            {data?.sections?.recent_decisions?.length ? (
              data.sections.recent_decisions.map((item) => (
                <div className="flex items-center justify-between gap-3 p-4 text-sm" key={item.id}>
                  <div>
                    <p className="font-bold text-slate-950">{item.food_handler}</p>
                    <p className="text-xs text-slate-500">{item.facility} · {dateLabel(item.signed_at)}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    {decisionBadge(item.decision)}
                    {item.return_to_work_date ? <span className="inline-flex items-center rounded bg-sky-100 px-2 py-0.5 text-xs font-bold text-sky-800">RTW: {dateLabel(item.return_to_work_date)}</span> : null}
                    <Link className="text-xs font-bold text-brand-green" href={`/doctor/assessments/${item.id}`}>View</Link>
                  </div>
                </div>
              ))
            ) : (
              <p className="p-4 text-sm text-slate-500">No recent decisions.</p>
            )}
          </div>
        </section>
        ...
        {data?.sections?.workload_summary?.length ? (
          <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-sm font-bold text-slate-950 mb-3">Workload Summary</h2>
            <div className="grid gap-3 md:grid-cols-3">
              {data.sections.workload_summary.map((item, i) => (
                <div className="rounded border border-slate-200 p-3 text-sm" key={i}>
                  <p className="font-bold text-slate-950">{item.status || item.category}</p>
                  <p className="text-2xl font-bold text-slate-950 mt-1">{item.count || item.total}</p>
                </div>
              ))}
            </div>
          </section>
