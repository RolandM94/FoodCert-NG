      ...
      <div className="grid gap-5">
        ...
        <section className="grid gap-3 md:grid-cols-4">
          <div className="rounded-lg border border-neutral-200 bg-white p-4 shadow-sm">
            ...
            <p className="mt-2 text-xs font-bold uppercase text-neutral-500">Assessments</p>
            <p className="text-2xl font-bold text-neutral-900">{assessments.length}</p>
          </div>
          <div className="rounded-lg border border-neutral-200 bg-white p-4 shadow-sm">
            <Stethoscope className="text-brand-700" size={18} />
            <p className="mt-2 text-xs font-bold uppercase text-neutral-500">Current status</p>
          ...
          <div className="rounded-lg border border-neutral-200 bg-white p-4 shadow-sm">
            ...
            <p className="mt-2 text-xs font-bold uppercase text-neutral-500">Appointment</p>
            <p className="text-sm font-bold text-neutral-900">{dateLabel(latest?.appointment_date || latest?.assessment_date)}</p>
          </div>
          <div className="rounded-lg border border-neutral-200 bg-white p-4 shadow-sm">
            <FileCheck2 className="text-brand-700" size={18} />
            <p className="mt-2 text-xs font-bold uppercase text-neutral-500">Next action</p>
            <p className="text-sm font-bold text-neutral-900">{latestSnapshot?.next_action.label || "Not started"}</p>
          </div>
        </section>

        <section className="rounded-lg border border-neutral-200 bg-white shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-neutral-200 p-4">
            <h2 className="text-sm font-bold text-neutral-900">Assessment History</h2>
              ...
              <tbody className="divide-y divide-neutral-200">
                {assessments.map((assessment) => (
                  <tr key={assessment.id}>
                    <td className="p-3"><p className="font-bold text-neutral-900">{assessment.facility_name || "Medical facility"}</p><p className="text-xs text-neutral-500">{dateLabel(assessment.created_at)}</p></td>
                    <td className="p-3"><p className="text-sm font-semibold text-neutral-700">{dateLabel(assessment.appointment_date || assessment.assessment_date)}</p><StatusBadge status={assessment.appointment_status || "not_booked"} /></td>
                    ...
                    <td className="p-3">
                      <div className="flex flex-wrap gap-2">
                        <Link className="rounded border border-neutral-200 px-3 py-1.5 text-xs font-bold text-neutral-700" href={`/food-handler/assessments/${assessment.id}`}>Open</Link>
                        {assessment.payment_status !== "success" ? (
                          <Link className="rounded bg-brand-600 px-3 py-1.5 text-xs font-bold text-white" href={`/food-handler/assessment/${assessment.id}/pay`}>Pay</Link>
                        ) : null}
                      </div>
                    </td>
                  </tr>
                ))}
                {!assessments.length && !loading ? <tr><td className="p-3 text-neutral-500" colSpan={5}>No assessments found.</td></tr> : null}
              </tbody>
            </table>
          </div>
        </section>
