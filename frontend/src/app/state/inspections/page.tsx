      ...
      <div className="grid gap-5">
        <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          <div className="grid gap-3 xl:grid-cols-[1fr_180px_220px_180px_auto]">
            <input className="h-10 rounded border border-slate-200 bg-slate-50 px-3 text-sm" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search employer, inspector, or findings" />
            <select className="h-10 rounded border border-slate-200 bg-white px-3 text-sm" value={queue} onChange={(event) => setQueue(event.target.value as typeof queue)}>
              <option value="">All queues</option>
              <option value="active">Active</option>
              <option value="submitted">Submitted</option>
              <option value="enforcement">Enforcement</option>
            </select>
            <select className="h-10 rounded border border-slate-200 bg-white px-3 text-sm" value={enforcementAction} onChange={(event) => setEnforcementAction(event.target.value as EnforcementAction | "")}>
              {enforcementOptions.map((option) => <option key={option.value || "all"} value={option.value}>{option.label}</option>)}
            </select>
            <select className="h-10 rounded border border-slate-200 bg-white px-3 text-sm" value={status} onChange={(event) => setStatus(event.target.value as InspectionStatus | "")}>
        ...
        <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
          <div className="grid gap-3">
            <div className="flex items-center gap-2"><ClipboardCheck className="text-brand-deep" size={18} /><h2 className="text-base font-bold text-slate-950">Inspection Workflow</h2></div>
            <DataTable<StateInspectionItem>
              columns={[
                { key: "employer", header: "Employer", render: (row) => <button className="text-left" onClick={() => setSelectedInspectionId(row.id)} type="button"><p className="font-bold text-slate-950">{row.employer_name}</p><p className="text-xs text-slate-500">{row.lga_name || "LGA not set"}</p></button> },
          ...
          <aside className="grid gap-4">
            <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              <div className="mb-3 flex items-center gap-2"><ClipboardList className="text-brand-deep" size={18} /><h3 className="text-sm font-bold text-slate-950">Assign Inspection</h3></div>
              <div className="grid gap-3">
                <select className="h-10 rounded border border-slate-200 bg-white px-3 text-sm" value={selectedInspectorId} onChange={(event) => setSelectedInspectorId(event.target.value)}>
                  <option value="">Select inspector</option>
                  {inspectors.map((user) => <option key={user.id} value={user.id}>{nameFor(user)}</option>)}
                </select>
                <select className="h-10 rounded border border-slate-200 bg-white px-3 text-sm" value={selectedEmployerId} onChange={(event) => setSelectedEmployerId(event.target.value)}>
                  <option value="">Select employer</option>
                  {(employersQuery.data || []).map((employer) => <option key={employer.id} value={employer.id}>{employer.business_name}</option>)}
                </select>
                <textarea className="min-h-20 rounded border border-slate-200 bg-slate-50 px-3 py-2 text-sm" value={assignmentFindings} onChange={(event) => setAssignmentFindings(event.target.value)} placeholder="Assignment note or scope" />
                <button className="h-10 rounded bg-brand-deep px-3 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:bg-slate-300" disabled={!selectedInspectorId || !selectedEmployerId || assignMutation.isPending} onClick={() => assignMutation.mutate()} type="button">
            ...
            <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              <div className="mb-3 flex items-center gap-2"><ShieldCheck className="text-brand-deep" size={18} /><h3 className="text-sm font-bold text-slate-950">Review & Close</h3></div>
              {selectedInspection ? (
                <div className="grid gap-3">
                  <div>
                    <p className="font-bold text-slate-950">{selectedInspection.employer_name}</p>
                    <p className="text-xs text-slate-500">{selectedInspection.inspector_name || "No inspector"} / {dateLabel(selectedInspection.inspection_date)}</p>
                  </div>
                  <select className="h-10 rounded border border-slate-200 bg-white px-3 text-sm" value={reviewAction} onChange={(event) => setReviewAction(event.target.value as EnforcementAction)}>
                    {enforcementOptions.filter((option) => option.value).map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
                  </select>
                  <textarea className="min-h-24 rounded border border-slate-200 bg-slate-50 px-3 py-2 text-sm" value={reviewFindings} onChange={(event) => setReviewFindings(event.target.value)} placeholder="Review findings or enforcement note" />
                  <button className="h-10 rounded border border-brand-deep px-3 text-sm font-semibold text-brand-deep disabled:cursor-not-allowed disabled:border-slate-300 disabled:text-slate-400" disabled={reviewMutation.isPending} onClick={() => reviewMutation.mutate()} type="button">
                    {reviewMutation.isPending ? "Saving..." : "Save review"}
                  </button>
                  <textarea className="min-h-20 rounded border border-slate-200 bg-slate-50 px-3 py-2 text-sm" value={closureNotes} onChange={(event) => setClosureNotes(event.target.value)} placeholder="Closure notes" />
                  <button className="h-10 rounded bg-slate-950 px-3 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:bg-slate-300" disabled={closeMutation.isPending || selectedInspection.status === "closed"} onClick={() => closeMutation.mutate()} type="button">
                    {closeMutation.isPending ? "Closing..." : "Close inspection"}
                  </button>
                  <div className="grid gap-2 border-t border-slate-100 pt-3">
                    {(selectedInspection.audit_history || []).slice(0, 4).map((log) => (
                      <div key={log.id} className="rounded border border-slate-100 bg-slate-50 p-2 text-xs text-slate-600">
                        <p className="font-semibold text-slate-800">{String(log.metadata.event || log.action).replaceAll("_", " ")}</p>
                ...
                <p className="text-sm text-slate-600">Select an inspection to review enforcement history.</p>
              )}
            </section>
          </aside>
        </section>
