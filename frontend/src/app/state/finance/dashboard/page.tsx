    ...
    <PortalShell role="state_admin" title="State finance" description="Monitor state-scoped payments, settlements, refunds, and reconciliation issues.">
      <div className="grid gap-5">
        <section className="flex items-center justify-between rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center gap-2 text-brand-deep"><Banknote size={18} /><h2 className="text-base font-bold text-slate-950">Finance Overview</h2></div>
          <button className="inline-flex items-center gap-2 rounded border border-slate-200 px-3 py-2 text-sm font-semibold" onClick={() => query.refetch()} type="button"><RefreshCw size={16} />Refresh</button>
        </section>
        <section className="grid gap-3 md:grid-cols-3 xl:grid-cols-5">
          {cardItems.map(([label, amount, count]) => (
            <div key={label} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              <p className="text-xs font-bold uppercase text-slate-500">{label}</p>
              <p className="mt-2 text-xl font-bold text-slate-950">{typeof amount === "number" && label === "Reconciliation issues" ? amount : money(amount)}</p>
