  ...
  return (
    <PortalShell role="facility_admin" title="Reports" description="Generate role-aware facility reports and export finance-safe operational summaries.">
      <div className="grid gap-5">
        {loading ? <p className="rounded-lg border border-neutral-200 bg-white p-4 text-sm font-semibold text-neutral-600 shadow-sm">Loading reports...</p> : null}
        ...
        <section className="rounded-lg border border-neutral-200 bg-white p-5 shadow-sm">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-neutral-900">Report builder</h2>
          </div>
          <div className="grid gap-3 lg:grid-cols-[170px_170px_190px_210px_150px_auto]">
            ...
            <select className="h-10 rounded border border-neutral-200 bg-white px-3 text-sm" value={fileFormat} onChange={(event) => setFileFormat(event.target.value as ReportFormat)}>
              {FORMAT_OPTIONS.map(([value, text]) => <option key={value} value={value}>{text}</option>)}
            </select>
            <button className="inline-flex h-10 items-center justify-center gap-2 rounded bg-brand-600 px-4 text-sm font-bold text-white disabled:opacity-60" disabled={busy || !facility} type="button" onClick={() => void generate()}><Send size={16} /> Generate</button>
          </div>
          <p className="mt-3 text-xs font-semibold text-neutral-500">Facility finance exports include settlement and operational counts only. Clinical notes, declaration answers, and lab result values are excluded.</p>
        </section>

        <section className="rounded-lg border border-neutral-200 bg-white shadow-sm">
          <div className="flex items-center justify-between gap-3 border-b border-neutral-200 p-4">
            <div>
              <h2 className="text-sm font-bold text-neutral-900">Recent generated reports</h2>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-neutral-200">
                {reports.length ? reports.map((report) => (
                  <tr key={report.id}>
                    <td className="p-3"><p className="font-bold capitalize text-neutral-900">{label(report.report_type)}</p><p className="text-xs text-neutral-500">{Object.keys(report.filters || {}).length} filters</p></td>
                    <td className="p-3 font-semibold uppercase text-neutral-700">{report.file_format}</td>
                    <td className="p-3"><StatusBadge status={report.status} /></td>
                    <td className="p-3 text-neutral-700">{report.generated_by_name || "System"}</td>
                    <td className="p-3 text-neutral-700">{formatDate(report.created_at)}</td>
                    <td className="p-3">{report.file_url ? <a className="inline-flex items-center gap-1 font-bold text-brand-700 underline" href={report.file_url} rel="noreferrer" target="_blank"><Download size={14} /> Download</a> : "Preview only"}</td>
                  </tr>
                )) : (
                  <tr><td className="p-3 text-neutral-500" colSpan={6}>No facility reports generated yet.</td></tr>
                )}
              </tbody>
            </table>
          </div>
