  ...
  return (
    <PortalShell role="facility_admin" title="Assessment Detail" description="Review workflow status, task panels, and state submission readiness.">
      <div className="grid gap-5">
        <Link className="inline-flex w-fit items-center gap-2 rounded border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-700 shadow-sm" href="/facility/assessments">
        ...
        {assessment ? (
          <>
            <section className="grid gap-3 md:grid-cols-4">
              ...
              <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs font-bold uppercase text-slate-500">Employer</p><p className="mt-2 font-bold text-slate-950">{assessment.employer_name || "Individual"}</p><p className="text-xs text-slate-500">{assessment.branch_name || "No branch"}</p></div>
              <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs font-bold uppercase text-slate-500">Appointment</p><p className="mt-2 font-bold text-slate-950">{formatDate(assessment.appointment_date)}</p><StatusBadge status={assessment.appointment_status || "not_booked"} /></div>
              <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs font-bold uppercase text-slate-500">Payment</p><p className="mt-2 font-bold text-slate-950">{label(assessment.payment_status)}</p><StatusBadge status={assessment.payment_status || "missing"} /></div>
            </section>

            <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
              <h2 className="text-sm font-bold text-slate-950">Workflow Stepper</h2>
              <div className="mt-4 grid gap-3 md:grid-cols-5">
                {[
                  ...
                  return (
                    <div className="rounded border border-slate-200 bg-slate-50 p-3" key={title as string}>
                      <StepIcon className="text-brand-deep" size={18} />
                      <p className="mt-2 text-xs font-bold uppercase text-slate-500">{title as string}</p>
                      <p className="text-sm font-bold capitalize text-slate-950">{label(status as string)}</p>
                    </div>
                  );
                })}
              </div>
            ...
            <section className="grid gap-4 lg:grid-cols-2">
              <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                <h2 className="text-sm font-bold text-slate-950">Declaration</h2>
                ...
                ) : <p className="mt-3 text-sm text-slate-500">Declaration details are unavailable for this role or not submitted yet.</p>}
              </div>
              ...
              <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                ...
                ) : <p className="mt-3 text-sm text-slate-500">No physical examination recorded.</p>}
              </div>

              <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                <h2 className="text-sm font-bold text-slate-950">Lab Tests</h2>
                <div className="mt-3 grid gap-2">
                  {assessment.lab_tests?.length ? assessment.lab_tests.map((test) => (
                    <div className="rounded border border-slate-100 bg-slate-50 p-3" key={test.id}>
                      <p className="font-bold text-slate-950">{test.test_name || label(test.test_type)}</p>
                      <StatusBadge status={test.status} />
                    </div>
                  )) : <p className="text-sm text-slate-500">No lab tests recorded or visible.</p>}
                </div>
              </div>

              <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                <h2 className="text-sm font-bold text-slate-950">Vaccination And State Submission</h2>
