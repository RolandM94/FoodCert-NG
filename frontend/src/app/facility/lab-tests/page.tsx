  ...
  return (
    <PortalShell role="facility_admin" title="Lab Tests" description="Track requested tests, sample collection, result entry, uploads, and doctor review status.">
      <div className="grid gap-5">
        {loading ? <p className="rounded-lg border border-slate-200 bg-white p-4 text-sm font-semibold text-slate-600 shadow-sm">Loading lab requests...</p> : null}
        <section className="grid gap-3 md:grid-cols-4">
          <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm"><FlaskConical className="text-brand-deep" size={18} /><p className="mt-2 text-xs font-bold uppercase text-slate-500">Requests</p><p className="text-2xl font-bold text-slate-950">{metrics.total}</p></div>
          <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm"><TestTube2 className="text-brand-deep" size={18} /><p className="mt-2 text-xs font-bold uppercase text-slate-500">Awaiting sample</p><p className="text-2xl font-bold text-slate-950">{metrics.requested}</p></div>
        ...
        <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-200 p-4"><h2 className="text-sm font-bold text-slate-950">Lab Request Queue</h2></div>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-200">
                ...
                )) : <tr><td className="p-3 text-slate-500" colSpan={6}>No lab requests found.</td></tr>}
              </tbody>
            </table>
          </div>
        </section>
