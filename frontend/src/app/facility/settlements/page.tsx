  ...
  return (
    <PortalShell role="facility_admin" title="Settlements" description="Track facility payouts, reconciliation totals, and settlement disputes without medical record details.">
      <div className="grid gap-5">
        {loading ? <p className="rounded-lg border border-neutral-200 bg-white p-4 text-sm font-semibold text-neutral-600 shadow-sm">Loading settlements...</p> : null}
        ...
        <section className="rounded-lg border border-neutral-200 bg-white p-4 shadow-sm">
          <div className="grid gap-3 lg:grid-cols-[170px_170px_190px_210px_auto_auto]">
            <input className="h-10 rounded border border-neutral-200 bg-neutral-50 px-3 text-sm" type="date" value={filters.date_from} onChange={(event) => updateFilter("date_from", event.target.value)} />
            <input className="h-10 rounded border border-neutral-200 bg-neutral-50 px-3 text-sm" type="date" value={filters.date_to} onChange={(event) => updateFilter("date_to", event.target.value)} />
            ...
            <select className="h-10 rounded border border-neutral-200 bg-white px-3 text-sm" value={filters.dispute_status} onChange={(event) => updateFilter("dispute_status", event.target.value)}>
              {DISPUTE_OPTIONS.map(([value, text]) => <option key={value || "all"} value={value}>{text}</option>)}
            </select>
            <button className="inline-flex h-10 items-center justify-center gap-2 rounded border border-neutral-200 px-3 text-sm font-bold text-neutral-700" type="button" onClick={() => void loadData()}><RefreshCw size={16} /> Apply</button>
            <button
              className="inline-flex h-10 items-center justify-center gap-2 rounded bg-brand-700 px-3 text-sm font-bold text-white disabled:opacity-60"
        ...
        <section className="rounded-lg border border-neutral-200 bg-white shadow-sm">
          <div className="border-b border-neutral-200 p-4">
            <h2 className="text-sm font-bold text-neutral-900">{facility?.facility_name || "Facility"} settlement ledger</h2>
          </div>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-neutral-200">
                {rows.length ? rows.map((row) => (
                  <tr key={row.id}>
                    <td className="p-3">
                      <p className="font-bold text-neutral-900">{row.payment_reference || row.settlement_reference || row.id}</p>
                      <p className="text-xs text-neutral-500">Allocation {row.payment_allocation_reference || row.payment_allocation || "legacy"}</p>
                      <p className="text-xs text-neutral-500">Fee {row.fee_schedule_name || "Not linked"} · Created {formatDate(row.created_at)}</p>
                    </td>
                    ...
                    <td className="p-3"><StatusBadge status={row.dispute_status} />{row.dispute_reason ? <p className="mt-1 max-w-48 text-xs text-neutral-500">{row.dispute_reason}</p> : null}</td>
                    <td className="p-3">
                      <button className="inline-flex h-9 items-center gap-2 rounded border border-neutral-200 px-3 text-xs font-bold text-neutral-700 disabled:opacity-60" disabled={row.dispute_status === "open" || row.dispute_status === "under_review"} type="button" onClick={() => setDisputeRow(row)}><FileWarning size={14} /> Dispute</button>
                    </td>
                  </tr>
                )) : (
                  <tr><td className="p-3 text-neutral-500" colSpan={8}>No settlement records match the current filters.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        {disputeRow ? (
          <section className="rounded-lg border border-warning-100 bg-warning-50 p-4 shadow-sm">
            ...
            <textarea className="mt-3 min-h-24 w-full rounded border border-warning-100 bg-white p-3 text-sm" placeholder="Describe the reconciliation issue" value={disputeReason} onChange={(event) => setDisputeReason(event.target.value)} />
            <div className="mt-3 flex flex-wrap gap-2">
              <button className="inline-flex h-10 items-center gap-2 rounded bg-brand-600 px-4 text-sm font-bold text-white disabled:opacity-60" disabled={busy || !disputeReason.trim()} type="button" onClick={() => void submitDispute()}><Send size={16} /> Submit dispute</button>
