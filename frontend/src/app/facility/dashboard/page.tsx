      ...
      <div className="grid gap-5">
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          <div className="grid gap-3 lg:grid-cols-[170px_170px_190px_210px_auto]">
            ...
            <button className="inline-flex h-10 items-center justify-center gap-2 rounded bg-brand-green px-3 text-sm font-bold text-white" type="button" onClick={() => void loadData()}><RefreshCw size={16} /> Refresh</button>
          </div>
        </section>
        ...
        <section className="grid gap-3 md:grid-cols-4">
          {[
            ...
            return (
              <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm" key={title as string}>
                <MetricIcon className="text-brand-deep" size={18} />
                <p className="mt-2 text-xs font-bold uppercase text-slate-500">{title as string}</p>
                <p className="text-xl font-bold text-slate-950">{typeof value === "number" ? numberLabel(value) : String(value ?? "0")}</p>
              </div>
            );
          })}
        </section>
        ...
        <section className="grid gap-4 lg:grid-cols-[0.8fr_1.2fr]">
          <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-sm font-bold text-slate-950">{facility?.facility_name || "Facility"} queues</h2>
            <div className="mt-4 grid gap-2">
              {queues.map((queue) => (
                <Link className="flex items-center justify-between rounded border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-700" href={String(queue.href || "/facility/dashboard")} key={String(queue.name)}>
                  <span>{queue.name}</span><span className="font-bold text-slate-950">{queue.count}</span>
                </Link>
              ))}
            </div>
            <div className="mt-4 rounded border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">
              ...
              <p className="mt-1 text-xs text-slate-500">Re-accreditation countdown: {cards.reaccreditation_countdown_days ?? "Not set"} days</p>
            </div>
          </div>

          <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-sm font-bold text-slate-950">Operational distributions</h2>
            <div className="mt-4 grid gap-4 md:grid-cols-3">
              {[["Assessments", assessmentStatus, "status"], ["Lab", labStatus, "lab_status"], ["Decisions", decisions, "final_decision"]].map(([title, rows, key]) => (
                <div className="rounded border border-slate-200 bg-slate-50 p-3" key={title as string}>
                  <p className="text-xs font-bold uppercase text-slate-500">{title as string}</p>
                  <div className="mt-2 grid gap-2">
                    {(rows as ChartRow[]).length ? (rows as ChartRow[]).map((row) => (
                      <div className="flex items-center justify-between text-xs" key={String(row[key as keyof ChartRow])}>
                        <span className="capitalize text-slate-600">{label(String(row[key as keyof ChartRow] || ""))}</span>
        ...
        <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-200 p-4">
            <h2 className="text-sm font-bold text-slate-950">Recent assessments</h2>
          </div>
              ...
              <tbody className="divide-y divide-slate-200">
                {recent.length ? recent.map((row) => (
                  <tr key={String(row.id)}>
                    <td className="p-3 font-bold text-slate-950">{row.food_handler}</td>
                    <td className="p-3"><StatusBadge status={String(row.status || "")} /></td>
                    <td className="p-3"><StatusBadge status={String(row.decision || "")} /></td>
                    <td className="p-3 text-slate-700">{row.doctor || "Unassigned"}</td>
                  </tr>
                )) : <tr><td className="p-3 text-slate-500" colSpan={4}>No recent assessments.</td></tr>}
              </tbody>
            </table>
          </div>
        </section>
