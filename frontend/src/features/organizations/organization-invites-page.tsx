...
function InviteRow({ invite, disabled, onRevoke }: { invite: UserInvite; disabled?: boolean; onRevoke: (invite: UserInvite) => void }) {
  return (
    <tr>
      <td className="border-b border-neutral-50 py-4 pr-4">
        <p className="font-semibold text-neutral-900">{invite.email}</p>
        <p className="mt-1 text-xs text-neutral-500">{invite.phone || "No phone"}</p>
      </td>
      <td className="border-b border-neutral-50 py-4 pr-4 text-sm font-semibold capitalize text-neutral-700">{invite.role.replaceAll("_", " ")}</td>
      <td className="border-b border-neutral-50 py-4 pr-4 text-sm text-neutral-600">{invite.unit_name || "Organization-wide"}</td>
      <td className="border-b border-neutral-50 py-4 pr-4 text-sm text-neutral-600">{invite.invited_by_name || invite.invited_by_email || "Unknown"}</td>
      <td className="border-b border-neutral-50 py-4 pr-4"><InviteStatusBadge status={invite.status} /></td>
      <td className="border-b border-neutral-50 py-4 pr-4 text-sm text-neutral-600">{formatDate(invite.expires_at)}</td>
      <td className="border-b border-neutral-50 py-4 text-right">
        <button
          className="h-9 rounded border border-neutral-200 px-3 text-sm font-bold text-neutral-700 hover:bg-neutral-50 disabled:opacity-50"
        ...
        <section className="rounded-lg border border-neutral-200 bg-white p-5 shadow-sm">
          <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-bold text-neutral-900">Invite List</h2>
            </div>
          ...
          {invitesQuery.isError ? <p className="rounded bg-danger-50 p-3 text-sm font-semibold text-danger-700">Could not load invites.</p> : null}
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px] text-left text-sm">
              <thead className="text-xs font-bold uppercase tracking-wide text-neutral-500">
                <tr>
                  <th className="border-b border-neutral-100 py-2 pr-4">Recipient</th>
                  <th className="border-b border-neutral-100 py-2 pr-4">Role</th>
                  <th className="border-b border-neutral-100 py-2 pr-4">Unit</th>
                  <th className="border-b border-neutral-100 py-2 pr-4">Invited By</th>
                  <th className="border-b border-neutral-100 py-2 pr-4">Status</th>
                  <th className="border-b border-neutral-100 py-2 pr-4">Expires</th>
              ...
              <tbody>
                ...
                {!invites.length && !invitesQuery.isFetching ? <tr><td className="py-8 text-center text-neutral-500" colSpan={7}>No invites have been sent yet.</td></tr> : null}
              </tbody>
            </table>
          </div>
        </section>
