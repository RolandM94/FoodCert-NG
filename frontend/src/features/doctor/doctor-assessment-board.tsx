...
type DoctorBoardFilter = "all" | "declaration" | "physical" | "lab" | "decision" | "review";

const TASK_FILTERS: Array<[DoctorBoardFilter, string]> = [
  ["all", "All assigned"],
...
function matchesFilter(row: MedicalAssessment, filter: DoctorBoardFilter) {
  ...
  if (filter === "review") return row.physical_exam_status === "completed" || row.lab_status === "reviewed" || row.final_decision === "pending";
  return true;
}

export function DoctorAssessmentBoard({
  title,
  description,
  initialFilter = "all",
  basePath = "/doctor/assessments",
}: {
  title: string;
  description: string;
  initialFilter?: DoctorBoardFilter;
  basePath?: string;
}) {
  const [assessments, setAssessments] = useState<MedicalAssessment[]>([]);
  const [filter, setFilter] = useState<DoctorBoardFilter>(initialFilter);
  ...
  const filtered = useMemo(() => assessments.filter((row) => matchesFilter(row, filter)), [assessments, filter]);

  return (
    <PortalShell role="doctor" title={title} description={description}>
      <div className="grid gap-5">
        ...
        <section className="flex flex-wrap items-center gap-3 rounded-lg border border-neutral-200 bg-white p-4 shadow-sm">
          <select className="h-10 rounded border border-neutral-200 bg-white px-3 text-sm" value={filter} onChange={(event) => setFilter(event.target.value as DoctorBoardFilter)}>
              ...
              <tbody className="divide-y divide-neutral-200">
                {filtered.length ? filtered.map((assessment) => (
                  <tr key={assessment.id}>
                    ...
                    <td className="p-3"><Link className="rounded border border-neutral-200 px-3 py-1.5 text-xs font-bold text-neutral-700" href={`${basePath}/${assessment.id}`}>Open</Link></td>
                  </tr>
                )) : (
