...
function decisionLabel(value: FitnessDecision) {
  return DECISION_OPTIONS.find((option) => option.value === value)?.label ?? value.replaceAll("_", " ");
}

export function DoctorAssessmentWorkspace({ assessmentId, backHref = "/doctor/assessments" }: { assessmentId: string; backHref?: string }) {
  ...
  const loadData = useCallback(async () => {
    ...
    setError("");
    try {
      const row = await getDoctorAssessment(assessmentId);
      const timelineRows = await getAssessmentAuditTimeline(assessmentId);
      setAssessment(row);
  ...
  return (
    <PortalShell role="doctor" title="Assessment Review" description="Validate health declaration risk, complete physical examination, and prepare next workflow steps.">
      <div className="grid gap-5">
        <Link className="inline-flex w-fit items-center gap-2 rounded border border-neutral-200 bg-white px-3 py-2 text-sm font-bold text-neutral-700 shadow-sm" href={backHref}>
          ...
          <>
            ...
            <section className="grid gap-4 lg:grid-cols-2">
              ...
              <div className="rounded-lg border border-neutral-200 bg-white p-5 shadow-sm">
                ...
                <PhysicalExamForm value={exam} busy={busy} completed={assessment.physical_examination?.is_completed} onChange={setExam} onSaveDraft={() => void saveExam()} onComplete={() => void completeExam()} />
              </div>
              ...
              <div className="rounded-lg border border-neutral-200 bg-white p-5 shadow-sm lg:col-span-2">
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  ...
                  />
                  <LabTestRequestForm additionalTests={additionalLabTests} includeRequired={includeRequiredLabTests} busy={busy} onAdditionalTestsChange={setAdditionalLabTests} onIncludeRequiredChange={setIncludeRequiredLabTests} onSubmit={() => void requestAssessmentLabTests()} />
                </div>
              </div>
                ...
                <div className="grid gap-4 lg:grid-cols-[0.8fr_1.2fr]">
                  <div className="grid gap-2">
                    {assessment.workflow_recommendation ? (
                      <div className={`rounded border p-3 text-sm ${assessment.workflow_recommendation.status === "ready" ? "border-brand-200 bg-brand-50 text-brand-900" : assessment.workflow_recommendation.status === "blocked" ? "border-danger-100 bg-danger-50 text-danger-800" : "border-warning-100 bg-warning-50 text-amber-900"}`}>
                        <p className="text-xs font-bold uppercase tracking-wide">System recommendation</p>
                        <p className="mt-2 font-bold">{decisionLabel(assessment.workflow_recommendation.suggested_decision)}</p>
                        <p className="mt-1">{assessment.workflow_recommendation.rationale}</p>
                        {assessment.workflow_recommendation.reasons.slice(1).length ? (
                          <ul className="mt-2 list-disc space-y-1 pl-4 text-xs">
                            {assessment.workflow_recommendation.reasons.slice(1).map((reason) => <li key={reason}>{reason}</li>)}
                          </ul>
                        ) : null}
                      </div>
                    ) : null}
                    ...
                    {assessment.workflow_recommendation ? (
                      <div className="grid gap-2 rounded border border-neutral-200 bg-neutral-50 p-3 text-xs text-neutral-700">
                        <div className="flex items-center justify-between gap-3"><span>Declaration risk</span><StatusBadge status={assessment.workflow_recommendation.signals.declaration_risk ? "warning" : "cleared"} /></div>
                        <div className="flex items-center justify-between gap-3"><span>Physical exam risk</span><StatusBadge status={assessment.workflow_recommendation.signals.physical_exam_risk ? "warning" : "cleared"} /></div>
                        <div className="flex items-center justify-between gap-3"><span>Flagged lab results</span><span className="font-bold text-neutral-900">{assessment.workflow_recommendation.signals.flagged_lab_results}</span></div>
                        <div className="flex items-center justify-between gap-3"><span>Repeat tests</span><span className="font-bold text-neutral-900">{assessment.workflow_recommendation.signals.repeat_required_tests}</span></div>
                        <div className="flex items-center justify-between gap-3"><span>Lab recommendation</span><span className="font-bold capitalize text-neutral-900">{assessment.workflow_recommendation.signals.highest_lab_recommendation.replaceAll("_", " ")}</span></div>
                        <div className="flex items-center justify-between gap-3">
                          <span>Vaccination concerns</span>
                          <span className="font-bold capitalize text-neutral-900">{assessment.workflow_recommendation.signals.vaccination_concerns.length ? assessment.workflow_recommendation.signals.vaccination_concerns.join(", ").replaceAll("_", " ") : "None"}</span>
                        </div>
                      </div>
                    ) : null}
                    ...
                    <div className="rounded border border-neutral-200 bg-neutral-50 p-3 text-sm text-neutral-700">Certificate request: <span className="font-bold">{assessment.can_request_certificate ? "Eligible" : "Not eligible yet"}</span></div>
                  </div>
                  <div className="grid gap-3">
                    <select className="h-10 rounded border border-neutral-200 bg-white px-3 text-sm" disabled={Boolean(assessment.signed_at)} value={decision.final_decision} onChange={(event) => setDecision((current) => ({ ...current, final_decision: event.target.value as FitnessDecision }))}>
                    ...
                    {["temporarily_not_fit", "requires_public_health_clearance", "return_to_work_on_date"].includes(decision.final_decision) ? <div className="rounded border border-warning-100 bg-warning-50 p-3 text-sm font-semibold text-amber-900">This decision will restrict food-handling duties and open return-to-work follow-up after sign-off.</div> : null}
